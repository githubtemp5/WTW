...
def make_model():
    model = Sequential()
    model.add(LSTM(18, input_shape=(1, 2), activation='relu', return_sequences=True))
...
forecasts = model.predict(test_examples)
...
def covert_prediction(forecasts):
    ...
    for prediction in range((len(predictions))):
        predictions = array(predictions[prediction])
        predictions = predictions.reshape(1, len(predictions))
        scaler2 = MinMaxScaler(feature_range=(-1, 1))
        invert_scale = scaler2.inverse_transform(predictions)
