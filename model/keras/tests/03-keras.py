...
def make_model():
    ...
    # hidden layer 1
    model.add(layers.Dense(name='HL_1', units=30))
    model.add(layers.BatchNormalization())
    model.add(layers.Activation(name='tanh', activation='tanh'))
    model.add(layers.Dropout(0.3))
    
    # hidden layer 2
    model.add(layers.Dense(name='HL_2', units=10))
    model.add(layers.BatchNormalization())
    model.add(layers.Activation(name='relu2', activation='relu'))
    model.add(layers.Dropout(0.3))
    
    #hidden layer 3
    model.add(layers.Dense(name='HL_3', units=3))
    model.add(layers.BatchNormalization())
    model.add(layers.Activation(name='relu3', activation='relu'))
    model.add(layers.Dropout(0.3))
