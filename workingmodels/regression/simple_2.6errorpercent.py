...
df = pd.read_csv('C:\\Users\\ChoudhuryMB\\Desktop\\keras-work\\using_dataset\\sales_existing_lapse.csv',
                 header=None,
                 sep=',')
...
def preprocess_features(df):
    selected_features = df[['year', 'months', 'sales']]
...
def preprocess_targets(df):
    output_target = df['existing_book']
...
test_examples = preprocess_features(df.tail(19))
test_targets = preprocess_targets(df.tail(19))
...
def outcome_pred(index):
    pred = pd.read_csv("C:\\Users\\ChoudhuryMB\\Desktop\\keras-work\\using_dataset\\sales_existing_lapse.csv",
                       header=None,
                       sep=',')
