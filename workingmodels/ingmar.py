...
def model_save(save_model_directory, model, n_lag, n_seq, n_test, n_epochs):
    """
...
def execute_new_model():
    ...
    print_actual_and_forecasts(n_test, n_seq, forecasts, actual)
           
    # evaluate forecasts
    evaluate_forecasts(actual, forecasts, n_lag, n_seq)
    
    #save_model
    model_save(save_model_directory, model, n_lag, n_seq, n_test, n_epochs)
...
def load_model():
    ...
    To execute a pre-existing model you need to follow the following steps:
        1. Change the load_model_directory
        2. Change the load_dataset_directory
            2.1. Make sure to transform the dataset to a valid 
        3. Execute load_model() by selecting the "option 2" in the menu
    """
    load_model_directory = 'C:\\Users\\ChoudhuryMB\\Desktop\\lstm-test\\2018-07-prediction-error2-6'
    load_dataset_directory = 'C:\\Users\\ChoudhuryMB\\Desktop\\lstm-test\\alvin-dataset-new.csv'
    
    series = read_csv(load_dataset_directory,  header=0, parse_dates = ['Year_Date'], index_col=0, squeeze=True, date_parser=parser)
    ...
    ##model = model.load(load_model_directory)
"""
def main():
    """
    OPTIONS MENU
    Allows to select differents tasks:
        ...
        2. Load model will allow you to use a existing model to make predictions
        3. Change the dataset will allow you to reformat the dataset so it can run using models
    """
    options = 'What would you like to do?'
    options += '\n Enter 1 to execute a new model'
    options += '\n Enter 2 to load a pre-saved model'
    options += '\n Enter 3 to format the dataset'
    options += '\n Your option: '
    ...
    if choice == 1:
        ...
        print("You selected to load a pre-existing model")
        load_model()
    elif choice == 3:
